...
class ProductTemplate(models.Model):
    ...
    def _compute_module_qrcode_table_theme(self):
        for record in self:
            qrcode_table_theme_installed = request.env['ir.module.module'].sudo().search([
                ('name', '=', 'qrcode_table_theme'),
                ('state', '=', 'installed')
            ])

            if qrcode_table_theme_installed:
                record.module_qrcode_table_theme = True
            else:
                record.module_qrcode_table_theme = False
...
class RestaurantTable(models.Model):
    ...
    barcode_url = fields.Char(
        compute='_compute_barcode_url', string='QR Encoded Message')
    ...
    def _compute_barcode_url(self):
        dom_name = self.env['ir.config_parameter'].sudo(
        ...
        for record in self:
            if record.floor_id and record.floor_id.pos_config_id:
                ...
                image = self.get_image(
                    record.barcode_url, code='QR', width=150, hight=150, hr=True)
                record.write({'qr_image': image, 'qr_image_download': image})
            else:
                record.barcode_url = False
...
class PosOrder(models.Model):
    ...
    def _order_fields(self, ui_order):
        ...
        res['is_table_order'] = ui_order.get('is_table_order', False)
        res['token'] = ui_order.get('token', '')
        return res
    ...
    def _process_order(self, order, draft, existing_order):
        ...
        if order_id:
            ...
            if po_order and po_order.is_table_order and po_order.state == 'paid':
                table_os = self.env['table.order'].search(
                    [('token', '=', po_order.token), ('state', '!=', 'done')])
                if table_os:
    ...
    def _export_for_ui(self, order):
        ...
        res['is_table_order'] = order.is_table_order
        return res
...
class PosOrderLine(models.Model):
    ...
    table_order_line_id = fields.Integer(string="Table Order Id", default=0)

    def _export_for_ui(self, orderline):
        res = super(PosOrderLine, self)._export_for_ui(orderline)
        res['table_order_line_id'] = orderline.table_order_line_id
        return res
...
class TableOrder(models.Model):
    ...
    lines = fields.One2many('table.order.line', 'order_id',
                            string='Order Lines', readonly=True, copy=True)
    is_table_order = fields.Boolean(string="Is Table Order", default=True)
    pricelist_id = fields.Many2one('product.pricelist', string='Pricelist',
                                   required=True, readonly=True, default=_default_pricelist)

    customer_uid = fields.Char(string="UID")
    place_date = fields.Datetime(string="Place Order Date")
    order_status = fields.Char(
        string="Order Status", compute="_compute_order_status")
    ...
    def get_table_order_lists(self, table_id):
        ...
        for rec in orders:
            ...
            for line in rec.lines:
                if line.state != 'draft':
                    order_lines += [{'id': line.id, 'state': line.state, 'product_id': line.product_id.id, 'description': line.description,
                                     'name': line.name, 'qty': line.qty, 'price_unit': line.price_unit, 'price_extra': line.price_extra, 'price': line.price_subtotal_incl, 'note': line.note}]
            if order_lines:
                data.update({
                    'line': order_lines
                })
    ...
    def change_table_accept_all_order(self, order_id):
        if order_id:
            ...
            line_ids = order_id.lines.filtered(
                lambda l: l.state in ['draft', 'confirm'])
            line_ids.write({'state': 'ordered'})
        return True

    @api.model
    def change_table_prepare_order(self, order_id):
        if order_id:
            order_id = self.browse(int(order_id))
            line_ids = order_id.lines.filtered(lambda l: l.state == 'ordered')
            line_ids.write({'state': 'prepared'})
        return True
    ...
    def _amount_line_tax(self, line, fiscal_position_id):
        ...
        taxes = taxes.compute_all(price, line.order_id.pricelist_id.currency_id,
                                  line.qty, product=line.product_id, partner=False)['taxes']
    ...
    def _onchange_amount_all(self):
        for order in self:
            ...
            order.amount_tax = currency.round(
                sum(self._amount_line_tax(line, False) for line in order.lines))
...
class TableOrderLine(models.Model):
    ...
    name = fields.Char(string="Name", compute='_compute_table_order_line_name')
    ...
    order_id = fields.Many2one(
        'table.order', string='Order Ref', ondelete='cascade')
    ...
    def _compute_table_order_line_name(self):
        for rec in self:
            ...
            if rec.product_id and rec.description:
                rec.name = rec.product_id.name + \
                    ' ' + '(' + rec.description + ')'
    ...
    def _compute_amount_line_all(self):
        ...
        taxes = tax_ids_after_fiscal_position.compute_all(
            price, self.order_id.pricelist_id.currency_id, self.qty, product=self.product_id.sudo(), partner=False)
        return {
    ...
    def _onchange_product_id(self):
        if self.product_id:
            ...
            #     price, self.product_id.sudo().taxes_id, tax_ids_after_fiscal_position, self.company_id)
            # self._onchange_qty()
    ...
    def _onchange_qty(self):
        if self.product_id.sudo():
            ...
            if (self.product_id.sudo().taxes_id):
                taxes = self.product_id.sudo().taxes_id.compute_all(price, self.order_id.pricelist_id.sudo(
                ).currency_id, self.qty, product=self.product_id.sudo(), partner=False)
    ...
    def change_table_order_state(self, line_id, change_state):
        if line_id and change_state:
            ...
            notifications.append(['table.order.line', self.env.uid, {
                                 'table_order_line_message': vals}])
